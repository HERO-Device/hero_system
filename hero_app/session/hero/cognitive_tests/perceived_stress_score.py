...
from hero.consultation.display_screen import DisplayScreen, Colours
from hero.consultation.questions import Question, pss_questions
...
class PSS:
    def __init__(self, size=pg.Vector2(1024, 600), parent=None, question_count=10, auto_run=False, preload_audio=False,
        ...
        if not os.path.isdir("temp/question_audio"):
            os.mkdir("temp/question_audio")
        if not os.path.isdir("temp/question_audio/pss"):
            os.mkdir("temp/question_audio/pss")
    ...
    def preload_audio(self):
        ...
        exit_audio_file = f'temp/question_audio/pss/exit.mp3'
        exit_audio.save(exit_audio_file)

        for idx, question in enumerate(self.questions):
            question_audio = gtts.gTTS(text=question.text, lang='en', slow=False)
            question_audio_file = f'temp/question_audio/pss/question_{idx}.mp3'
