"""
Memory Test Module - Visual comparison memory assessment

Shows two images sequentially and asks if they're the same or different.
Tests short-term visual memory and attention to detail.
"""
...
class MemoryTest:
    """
    Visual memory test - comparing sequential images.
    ...
    def __init__(self, parent=None, auto_run=False, num_trials=32,
                 image_folder="games/Images", answers_file="games/shapesanswers.xlsx"):
        """
    ...
    def get_image_sets(self):
        ...
        # image1_1.jpg, image1_2.jpg, image2_1.jpg, image2_2.jpg, etc.
        return [f"image{i}" for i in range(1, min(self.num_trials + 1, 33))]
    ...
    def load_image_pair(self, image_set):
        ...
        Args:
            image_set: Name of image set (e.g., "image1")
        ...
        try:
            ...
            if not os.path.exists(img1_path) or not os.path.exists(img2_path):
                print(f"Warning: Images not found for {image_set}")
                return None, None
    ...
    def show_instruction(self):
        ...
        instruction = (
            "You will see two images, one after another.\n"
            "Click 'Same' if they are identical.\n"
            "Click 'Different' if they are not identical."
        )
        ...
        if self.parent and not self.auto_run:
            ...
            if self.parent.config.speech:
                self.parent.speak_text(
                    ...
                    "Tell me if they are the same or different."
                )
    ...
    def run_trial(self, image_set, trial_num):
        """
        Run a single trial of the memory test.
    ...
    def loop(self):
        ...
        avg_response_time = sum(r['response_time'] for r in self.responses) / len(
            self.responses) if self.responses else 0
...
if __name__ == "__main__":
    print("=" * 50)
    print("Memory Test - Standalone Mode")
    print("=" * 50)
    print("Compare sequential images\n")

    pg.init()
    test = MemoryTest(num_trials=3)  # Just 3 trials for testing
