...
# ── Velocity-adaptive double EMA parameters ───────────────────────────────────
# ALPHA_SLOW: used during fixations to suppress jitter (replaces old SMOOTHING_ALPHA)
# ALPHA_FAST: used when a saccade is detected (large jump in raw prediction)
# SACCADE_THRESHOLD: Euclidean pixel distance that triggers the fast alpha

ALPHA_SLOW = 0.12          # equivalent to old SMOOTHING_ALPHA = 0.15, slightly more stable
ALPHA_FAST = 0.4           # snaps cursor quickly during intentional gaze shifts
SACCADE_THRESHOLD = 60     # pixels — lower = more sensitive to saccades; tune if needed
...
def extract_features(landmarks, img_w, img_h):
    ...
    def norm_iris(iris_idx, inner_idx, outer_idx):
        ...
        return (ix - outx) / eye_w, ((iy - eye_cy)*7) / eye_w  ## adding a weight to boost the y values
...
def run():
    ...
    with dai.Device(pipeline) as device:
        ...
        # Use float state to avoid rounding drift in the EMA accumulation
        gaze_x = float(screen_w // 2)
        gaze_y = float(screen_h // 2)
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                # ── Velocity-adaptive double EMA ──────────────────────────────
                # If the raw prediction jumps further than SACCADE_THRESHOLD pixels
                # from the current smoothed position, use ALPHA_FAST so the cursor
                # snaps quickly to the new fixation.  Otherwise use ALPHA_SLOW to
                # keep jitter suppressed during stable fixations.
                dist  = np.hypot(raw_x - gaze_x, raw_y - gaze_y)
                alpha = ALPHA_FAST if dist > SACCADE_THRESHOLD else ALPHA_SLOW

                gaze_x = alpha * raw_x + (1.0 - alpha) * gaze_x
                gaze_y = alpha * raw_y + (1.0 - alpha) * gaze_y

            # Crosshair + dot gaze cursor

            cx, cy = int(gaze_x), int(gaze_y)

            cv2.circle(canvas, (cx, cy), CURSOR_RADIUS, (0, 255, 0), 2)

            cv2.line(canvas,

                     (cx - CURSOR_RADIUS, cy),

                     (cx + CURSOR_RADIUS, cy), (0, 255, 0), 1)

            cv2.line(canvas,

                     (cx, cy - CURSOR_RADIUS),

                     (cx, cy + CURSOR_RADIUS), (0, 255, 0), 1)
